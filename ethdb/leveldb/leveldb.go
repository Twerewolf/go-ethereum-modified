...
//Database是一个持久的键值存储。除了基础数据存储
//它还支持批量写入batch write和在键空间上以二进制字母顺序迭代
...
func New(file string, cache int, handles int, namespace string, readonly bool) (*Database, error) {
	log.Trace("TJW: ethdb/leveldb: New()=======================")
...
func NewCustom(file string, namespace string, customize func(options *opt.Options)) (*Database, error) {
	log.Trace("TJW: ethdb/leveldb: NewCustum()=======================")
...
func (db *Database) Close() error {
	log.Trace("TJW: ethdb/leveldb: Close()=======================")
	db.quitLock.Lock()
...
//TJW：在ethdb:levelDB Has()增加trace
func (db *Database) Has(key []byte) (bool, error) {
	log.Trace("TJW: ethdb/leveldb: Has(key []byte) (bool, error)=======================")
	return db.db.Has(key, nil)
}

// Get retrieves the given key if it's present in the key-value store.
//TJW：在ethdb:levelDB Get()增加trace
func (db *Database) Get(key []byte) ([]byte, error) {
	log.Trace("TJW: ethdb/leveldb: Get(key []byte) ([]byte, error)=======================")
...
func (db *Database) Put(key []byte, value []byte) error {
	log.Trace("TJW: ethdb/leveldb: Put(key []byte, value []byte) error=======================")
	return db.db.Put(key, value, nil)
}

// Delete removes the key from the key-value store.
func (db *Database) Delete(key []byte) error {
	log.Trace("TJW: ethdb/leveldb: Delete(key []byte) error=======================")
...
func (db *Database) NewBatch() ethdb.Batch {
	log.Trace("TJW: ethdb/leveldb: NewBatch() ethdb.Batch=======================")
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewIterator(prefix []byte, start []byte) ethdb.Iterator {
	log.Trace("TJW: ethdb/leveldb: NewIterator(prefix []byte, start []byte) ethdb.Iterator =======================")
	return db.db.NewIterator(bytesPrefixRange(prefix, start), nil)
}

// Stat returns a particular internal stat of the database.
func (db *Database) Stat(property string) (string, error) {
	log.Trace("TJW: ethdb/leveldb: Stat(property string) (string, error)=======================")
...
func (db *Database) Compact(start []byte, limit []byte) error {
	log.Trace("TJW: ethdb/leveldb: Compact(start []byte, limit []byte) error=======================")
	return db.db.CompactRange(util.Range{Start: start, Limit: limit})
}

// Path returns the path to the database directory.
func (db *Database) Path() string {
	log.Trace("TJW: ethdb/leveldb: Path() string=======================")
	return db.fn
}
...
func (db *Database) meter(refresh time.Duration) {
	...
	log.Trace("TJW: ethdb/leveldb: meter(refresh time.Duration)=======================")
