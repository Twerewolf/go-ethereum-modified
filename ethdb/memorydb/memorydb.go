...
//TJW: memdb是一个短暂kv存储，与基础的存储功能不同，
//memdb还提供batch writes组写入 和 二进制字母序中对keyspace的迭代？？
type Database struct {
	// map是比较复杂的数据结构，他的变量其实就是一个指针，占用8字节。
	// make返回的就是类型变量。要么是指针(map|channel)占用8字节，要么是(slice)占用24字节。
	db map[string][]byte

	lock sync.RWMutex //map是非并发安全的，使用过程中需要自己去控制加锁。
}
...
// // 增加memory-database 的大小
// func (db *Database) Size() int {
// 	db.lock.RLock()
// 	defer db.lock.RUnlock()
// 	size, _ := getRealSizeOf(db)
...
// kv元组，外加一个deletion标志，方便batch形式存一组kv对进入memory-database
